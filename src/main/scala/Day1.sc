...
//PARAMETERS 2
def printString2(message:String = "No string passed") = println(message)

printString2()
printString2("Hello world")

//PARAMETERS 3
def multiplyString(message:String, multiply:Int = 1) = println(message * multiply)
...
def formatDetails(name:String, address:String) = {
  println(s"My name is ${name.toUpperCase} and I work at ${address.toLowerCase}")
}

formatDetails("Marianne Pearson", "5th Floor, Disneyland Paris, YU56GHT")

//STRINGS 4
def formatNumber(number:Double) = println(f"${number}%.3f")
println("#####################")
formatNumber(145.789776785)

//STRINGS 5
def revString(message:String) = println(message.reverse)
...
//OPERATORS2
def ageInSeconds(age:Int) = println(f"You are $age years old, \nso your age in seconds is ${age*365*24*60*60} seconds")
